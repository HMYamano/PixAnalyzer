...
def select_threshold(image):
    ...
    layout = [
        ...
        [sg.Text("Lower threshold:"), sg.Slider((0, 255), key="-LOWER THRESHOLD-", orientation="h", size=(40, 15), default_value=100)],
        [sg.Text("Upper threshold:"), sg.Slider((0, 255), key="-UPPER THRESHOLD-", orientation="h", size=(40, 15), default_value=200)],
    ...
    while True:
        ...
        #         window["-IMAGE-"].update(data=to_bytes(original_image))
            ...
            # image = cv2.imread(image_path)
            processed_image = apply_threshold(image, int(values["-LOWER THRESHOLD-"]), int(values["-UPPER THRESHOLD-"]))
